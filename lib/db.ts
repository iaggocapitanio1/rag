...
type SimilarityDistance = "cosine" | "euclidean" | "dot_product";
  ...
  const db = client.db(Settings.astraDbUrl, {
  ...
  const createCollection = async (distance: SimilarityDistance = 'dot_product') => {
  ...
  const loader = new PuppeteerWebBaseLoader(url, {
    ...
    evaluate: async (page, browser) => {
      const text = await page.evaluate(() => {
        const unwantedTags = ['script', 'style', 'noscript', 'header', 'footer', 'nav', 'aside'];
...
createCollection().then(() => loadSampleData()).catch((err) => {
